...
/**
 * REST controller for managing {@link com.karim.irrigation.domain.Crop}.
 */
@RestController
@RequestMapping("/api")
public class CropResource {
     ...
     */
    @PostMapping("/crops")
    public ResponseEntity<Crop> createCrop(@Valid @RequestBody Crop crop) throws URISyntaxException {
     ...
     */
    @PutMapping("/crops/{id}")
    public ResponseEntity<Crop> updateCrop(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Crop crop)
        ...
        Crop result = cropService.update(crop);
        return ResponseEntity
            .ok()
